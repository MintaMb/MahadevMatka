...
export default function BidHistory(props) {
  ...
  useEffect(() => {
    ...
    get_data(lastMonthDate)
  }, [props])

  const get_data = async (fromDate) => {
      ...
      {history.length > 0 ?
        <FlatList
          ...
          renderItem={({ item, index }) => (
            ...
            <View style={[StyleConstants.cardView, {height:100, alignSelf: 'center', marginTop: 10, borderRadius: 10, width: screenWidth - 20, padding: 10 }]}>
              <Text style={{ fontSize: 20, fontWeight: 'bold', color: currentTheme().textColor }}>{item.game_name}</Text>
              ...
              <View style={{ flexDirection: 'row' }}>
                ...
                <View style={{ flex: 3, padding: 10, }}>
                  <Text style={{ fontSize: 16, fontWeight: 'bold', color: currentTheme().Blue, }}>{item.game_type}</Text>
                  <Text style={{ fontSize: 12, fontWeight: 'bold', color: currentTheme().themeColor, }}>
                ...
                <View style={{ flex: 3, padding: 10 }}>
                  <Text style={{ fontSize: 14, color: currentTheme().themeColor, alignSelf: 'center' }}>Session: {item.session}</Text>

                  <Text style={{ fontSize: 18, color: currentTheme().themeColor, alignSelf: 'center' }}>{item.points_action} Points</Text>
